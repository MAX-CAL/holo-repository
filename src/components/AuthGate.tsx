...
export function AuthGate({ onSuccess }: AuthGateProps) {
  ...
  const { loginWithCode, createUniverse } = useAuth();
        ...
        {!showCreatePrompt ? (
          <form onSubmit={handleSubmit} className="w-full space-y-4">
            <Input
              ...
              className="h-14 bg-card/50 border-border/50 text-center text-lg placeholder:text-muted-foreground/50 focus:border-primary/50 focus:ring-primary/20 transition-all"
              autoFocus
              autoComplete="off"
            />
